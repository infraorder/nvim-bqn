...
M.start_repl = function()
  ...
  M.buffers[buffer_name] = {
    tmp_file        = os.tmpname():gsub("(.*)/.*$","%1") .. '/' .. buffer_name,
    port            = util.get_port_number(),
    pending         = {},
    line_cache      = {}
  }

  vim.fn.jobstart({"bqn", "-i", M.buffers[buffer_name].port})
    ...
    uv.read_start(client, function (err, chunk)
      ...
      if (M.buffers[buffer_name].current ~= nil) then
        M.buffers[buffer_name].response = {
          type = chunk,
          output = table.concat(lines_from(M.buffers[buffer_name].tmp_file), "\n"),
...
M.schedule_first = function()
  ...
  M.send(M.buffers[buffer_name].tmp_file, M.buffers[buffer_name].current.data)
end
...
M.send = function(path, data)
  local buffer_name = util.get_buffer_name()
  uv.write(M.buffers[buffer_name].client, path .. ":" .. data)
end
